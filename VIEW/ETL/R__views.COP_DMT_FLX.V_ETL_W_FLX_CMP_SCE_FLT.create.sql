...
CREATE OR REPLACE VIEW V_ETL_W_FLX_CMP_SCE_FLT
...
,FINAL AS (
    SELECT   R_FLX_SCE.CBU_COD
            ...
            ,W_FLX_CMP_SCE.CFG_MOD_FLG
            ,R_FLX_GRP_IND.IND_NUM_FLG
            ,R_FLX_GRP_IND.IND_DEN_FLG
            ,W_FLX_CMP_SCE.PERIOD_ELEMENT_CODE                      AS PER_ELM_COD
            ,(CASE WHEN W_FLX_CMP_SCE.PERIOD_ELEMENT_CODE <= R_FLX_SCE.LST_ACT_PER_COD THEN 1 
                   ELSE 0 
              END)                                                  AS ACT_PER_FLG
            ,W_FLX_CMP_SCE.ENTITY_ELEMENT_CODE                      AS ETI_ELM_COD
            ,W_FLX_CMP_SCE.CUSTOMER_ELEMENT_CODE                    AS CUS_ELM_COD
            ,W_FLX_CMP_SCE.PRODUCT_ELEMENT_CODE                     AS PDT_ELM_COD
            ,W_FLX_CMP_SCE.MGR_L500_CODE                            AS CAT_TYP_ELM_COD
            ,(CASE WHEN R_FLX_SCE.EIB_USE_FLG = 0 THEN 'NON_EIB'
                   ELSE W_FLX_CMP_SCE.EIB_ELEMENT_CODE
              END)                                                  AS EIB_ELM_COD
            ,(CASE WHEN R_FLX_SCE.TTY_USE_FLG = 0 THEN 'NA'
                   ELSE W_FLX_CMP_SCE.TERRITORY_ELEMENT_CODE
              END)                                                  AS TTY_ELM_COD
            ,W_FLX_CMP_SCE.SU_SP_SPLIT_CODE                         AS SAL_SUP_ELM_COD
            ,W_FLX_CMP_SCE.CUR_COD                                  AS SRC_CUR_COD
            ,R_FLX_SCE.CUR_COD                                      AS SCE_CUR_COD
            ,(CASE R_FLX_SCE.CUR_COD
                   WHEN 'EUR_CY' THEN W_FLX_CMP_SCE.RATE_CY
                   WHEN 'EUR_FY' THEN W_FLX_CMP_SCE.RATE_FY
                   WHEN 'LC' THEN 1
              END)                                                  AS CNV_CUR_RAT
            ,(CASE R_FLX_SCE.CUR_COD
                   WHEN 'EUR_CY' THEN W_FLX_CMP_SCE.AMOUNT_EUR_CY
                   WHEN 'EUR_FY' THEN W_FLX_CMP_SCE.AMOUNT_EUR_FY
                   WHEN 'LC' THEN W_FLX_CMP_SCE.AMOUNT_LC
              END) * W_FLX_CMP_SCE.CFG_MOD_FLG                      AS AMOUNT
            ,W_FLX_CMP_SCE.ACCOUNT_ELEMENT_CODE
            ,W_FLX_CMP_SCE.DESTINATION_ELEMENT_CODE
            ,W_FLX_CMP_SCE.FUNCTIONAL_AREA_ELEMENT_CODE
            ,W_FLX_CMP_SCE.CATEGORY_ELEMENT_CODE
            ,W_FLX_CMP_SCE.CHANNEL_ELEMENT_CODE
            ,W_FLX_CMP_SCE.IOM_CODE
            ,W_FLX_CMP_SCE.PLANT_CODE
            ,W_FLX_CMP_SCE.ORIGINAL_ACCOUNT_ELEMENT_CODE
            ,W_FLX_CMP_SCE.T_REC_SRC_TST
    FROM     COP_DMT_FLX.R_FLX_SCE
             ...
             (
                  FLX_SCE.SCE_ELM_KEY = R_FLX_SCE.SCE_ELM_KEY
             )
             INNER JOIN COP_DMT_FLX.W_FLX_CMP_SCE ON 
             (
                  W_FLX_CMP_SCE.CBU_COD                     = FLX_SCE.CBU_COD                  AND
                  W_FLX_CMP_SCE.SCENARIO_ELEMENT_CODE       = FLX_SCE.SRC_SCE_ELM_COD          AND
                  W_FLX_CMP_SCE.PERIOD_ELEMENT_CODE   BETWEEN P_FLX_SCE_CFG_IND.BGN_PER_ELM_COD AND P_FLX_SCE_CFG_IND.END_PER_ELM_COD
             )
             -- To retrieve the volume unit configured for the Entity
             INNER JOIN COP_DMT_FLX.R_FLX_ETI ON 
             (
                  R_FLX_ETI.CBU_COD     = W_FLX_CMP_SCE.CBU_COD             AND
                  R_FLX_ETI.ETI_ELM_COD = W_FLX_CMP_SCE.ENTITY_ELEMENT_CODE
             )
             INNER JOIN COP_DMT_FLX.R_FLX_GRP_IND ON 
             (
                R_FLX_GRP_IND.IND_GRP_COD = P_FLX_SCE_CFG_IND.IND_ELM_COD AND
                R_FLX_GRP_IND.IND_ELM_COD = W_FLX_CMP_SCE.INDICATOR_CODE
                ...
                --            W_FLX_CMP_SCE.INDICATOR_CODE = 'VOL_'|| R_FLX_ETI.VOL_UNT_COD 
                --      ELSE R_FLX_GRP_IND.IND_ELM_COD = W_FLX_CMP_SCE.INDICATOR_CODE
                --  END)
             )
    WHERE    1 = 1
    ...
    OR        EXISTS (SELECT 1
                     ...
                     AND    R_FLX_GRP_ETI.ETI_ELM_COD = W_FLX_CMP_SCE.ENTITY_ELEMENT_CODE
                    )
            )
    ...
    OR        EXISTS (SELECT 1
                     ...
                     AND    R_FLX_GRP_CUS.CUS_ELM_COD = W_FLX_CMP_SCE.CUSTOMER_ELEMENT_CODE
                    )
            )
    ...
    OR        EXISTS (SELECT 1
                     ...
                     AND    R_FLX_GRP_PDT.PDT_ELM_COD = W_FLX_CMP_SCE.PRODUCT_ELEMENT_CODE
                    )
            )
    ...
    OR        EXISTS (SELECT 1
                     ...
                     WHERE R_FLX_GRP_CAT_TYP.CAT_TYP_GRP_COD = P_FLX_SCE_CFG_IND.CAT_TYP_ELM_COD
                         AND R_FLX_GRP_CAT_TYP.CAT_TYP_ELM_COD = W_FLX_CMP_SCE.MGR_L500_CODE
                     )
            )
    ...
    OR        EXISTS (SELECT 1
                     ...
                     AND    R_FLX_GRP_EIB.EIB_ELM_COD = W_FLX_CMP_SCE.EIB_ELEMENT_CODE
                    )
            )
    ...
    OR        EXISTS (SELECT 1
                     ...
                     AND    R_FLX_GRP_TTY.TTY_ELM_COD = W_FLX_CMP_SCE.TERRITORY_ELEMENT_CODE
                    )
            )
    ...
    OR        EXISTS (SELECT 1
                     ...
                     AND    R_FLX_GRP_SAL_SUP.SAL_SUP_ELM_COD = W_FLX_CMP_SCE.SU_SP_SPLIT_CODE
                    )
            )
...
--                                                              ,W_FLX_CMP_SCE.INDICATOR_CODE
            ...
            -- but we want to keep all the versions for denominators used to calculate ratios (they will be discarded after calculation).
                                                              ...
                                                              ,W_FLX_CMP_SCE.PERIOD_ELEMENT_CODE
                                                              ,W_FLX_CMP_SCE.ENTITY_ELEMENT_CODE
                                                              ,W_FLX_CMP_SCE.CUSTOMER_ELEMENT_CODE
                                                              ,W_FLX_CMP_SCE.PRODUCT_ELEMENT_CODE
                                                              ,W_FLX_CMP_SCE.MGR_L500_CODE
                                                              ,(CASE WHEN R_FLX_SCE.EIB_USE_FLG = 0 THEN 'NON_EIB'
                                                                     ELSE W_FLX_CMP_SCE.EIB_ELEMENT_CODE
                                                                END)
                                                              ,(CASE WHEN R_FLX_SCE.TTY_USE_FLG = 0 THEN 'NA'
                                                                     ELSE W_FLX_CMP_SCE.TERRITORY_ELEMENT_CODE
                                                                END)
                                                              ,W_FLX_CMP_SCE.SU_SP_SPLIT_CODE
                                                              )
)
SELECT   CBU_COD
        ,SCE_ELM_COD
