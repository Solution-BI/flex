...
CREATE OR REPLACE PROCEDURE SP_Flex_Ini_Agg()
    RETURNS VARCHAR
    LANGUAGE SQL
    EXECUTE AS OWNER
AS
...
=========================================================================
                                HISTORY
=========================================================================
Description : Create Procedure Script to load data in F_FLX_SRC_SCE
...
BEGIN
    ...
    CALL COP_DMT_FLX.SP_T_FLX_FLW_RUN_LOG_INIT(:V_RUN_ID,'COP_DMT_FLX','SP_Flex_Ini_Agg','#','DLT',CURRENT_USER);
    ...
    WHERE  (CBU_COD
           ,SCE_ELM_COD) IN (SELECT R_FLX_SCE.CBU_COD
    ...
    FROM        COP_DMT_FLX.W_FLX_SRC_SCE__GAP;
...
EXCEPTION
    WHEN OTHER THEN
         ...
         -- Update all scenarios to failed in case of failure
